...
func ParseCPE23(cpeString string) (*CPE, error) {
	// 添加 CPE 2.3 版本的解析逻辑
	// 示例正则表达式
	cpeRegex := regexp.MustCompile(`^cpe:2\.3:([aho]?):([*\p{Han}a-zA-Z0-9._%+-]+):([*\p{Han}a-zA-Z0-9._%+-]+):([*\p{Han}a-zA-Z0-9._%+-]+)(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?(?::([*a-zA-Z0-9._%+-]+|[*]))?$`)
	// 使用正则表达式匹配 CPE 字符串
